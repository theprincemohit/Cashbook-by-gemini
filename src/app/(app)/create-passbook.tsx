...
import { createPassbook } from '@/lib/passbooks';
import {
  AppColors,
  AppSpacing,
  AppBorderRadius,
...
export default function CreatePassbookScreen() {
  ...
  const handleCreate = async () => {
    const trimmed = passbookName.trim();

    if (!trimmed) {
      setError('Please enter a passbook name.');
      return;
    }
    if (trimmed.length < 2) {
      setError('Passbook name must be at least 2 characters.');
      return;
    }
    if (!businessId) {
      setError('Business not found. Please go back and try again.');
      return;
    }
    ...
    const result = await createPassbook(businessId, trimmed);
        ...
        >
          ...
          >
            ...
            <View style={styles.content}>
              ...
              <Text style={styles.title}>New Passbook</Text>
              <Text style={styles.subtitle}>
                Create a passbook under{' '}
              ...
              <View style={styles.inputGroup}>
                ...
                <View style={styles.inputWrapper}>
                  ...
                  <TextInput
                    style={styles.input}
                    placeholder="e.g. Sales, Purchases, Daily Cash"
                    placeholderTextColor={AppColors.textPlaceholder}
                    value={passbookName}
                    onChangeText={(text) => {
                      setPassbookName(text);
                      if (error) setError('');
                    }}
